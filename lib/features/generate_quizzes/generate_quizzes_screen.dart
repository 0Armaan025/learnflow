...
import 'package:learnflow/features/generate_quizzes/quiz_template_page.dart';
...
class GenerateQuizzesScreen extends StatefulWidget {
  const GenerateQuizzesScreen({Key? key}) : super(key: key);
...
class _GenerateQuizzesScreenState extends State<GenerateQuizzesScreen> {
  ...
  void extractText() async {
    ...
    setState(() {});
    // generateNotes(_generatedNotes);a
  }
  ...
  void makeNotes(BuildContext context) {
    if (imageFile == null && _studyContentController.text.isNotEmpty) {
      ...
      // generateNotes(scannedText);
      moveScreen(context, QuizTemplatePage(data: scannedText,));
        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              Center(
                child: Text(
                  "I guess, it's time for a quiz? :D",
